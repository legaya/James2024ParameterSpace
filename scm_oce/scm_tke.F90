...
subroutine tke_stp( e3t     ,      &            ! Cell thickness           [m]
                    ...
                    c_mu    ,      &            ! Coeff in the equation for avm
                    c_mu_prime ,   &            ! Coeff in the equation for avt
                    r_D     ,      &
       ...
       real(8),dimension( 0:jpk             ), intent(inout) :: lmix
       real(8),dimension( 0:jpk             ), intent(inout) :: eps
       real(8),dimension( 0:jpk             ), intent(inout) :: c_mu
       real(8),dimension( 0:jpk             ), intent(inout) :: c_mu_prime
! Grid variables
...
! local variables
       ...
       DO jk = 1, jpkm1           !* Matrix and right hand side in tke
          ...
          eps(jk)      = rn_ediss * dissl(jk,nstp)*tke(jk,nstp) ! rn_ediss is the "normal" zfact3 if we dont use numerical trick
          !
       END DO
       ...
       DO jk = 0, jpk            !* vertical eddy viscosity & diffivity at w-points
          ...
          lmix(jk)       = zmxlm(jk)
          c_mu(jk)       = rn_ediff
          c_mu_prime(jk) = rn_ediff * apdlr(jk)
       END DO
